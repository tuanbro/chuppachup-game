...
public class AnswerScreen extends MyScreen{
	...
	public AnswerScreen() {
		...
		Color myBlueColor = new Color(0.204f, 0.286f, 0.353f, 1);
		//Font Mark
		...
		_fontQuestion.setColor(myBlueColor);
		//Font Answer
		...
		_fontAnswer.setColor(myBlueColor);
		//Font Point
		...
		_fontPoint.setColor(myBlueColor);
		//Load IMG
		...
		for(int i = 1; i <= totalQuestion.length; i++){
			...
			question = new Question(_texQuestion, new Rectangle(ChupaChupGame.x, ChupaChupGame.y, WIDTH, HEIGHT),
					                              new Rectangle(120 + ChupaChupGame.x, 725 + ChupaChupGame.y, 0, 0),
					                              true, _fontQuestion, i, properties, answers);
			...
			for(int j = 1; j <= totalAnswer; j++){
				...
				int x = position[0];
				int y = position[1];
				answers.add(new Answer(_texAnswer, new Rectangle(x + ChupaChupGame.x - ANSWER_X_PLUS, y + ChupaChupGame.y - ANSWER_Y_PLUS, _texAnswer.getWidth(), _texAnswer.getHeight()),
                        						   new Rectangle( x + ChupaChupGame.x, y + ChupaChupGame.y, 0, 0),
                        						   _fontAnswer, j, answerProperties.nextToken(),
                        						   new MyActor(new Rectangle(x + ChupaChupGame.x + POINT_X_PLUS, y + ChupaChupGame.y - POINT_Y_PLUS, 0, 0), _fontPoint, answerProperties.nextToken())));
			}
	...
	public void draw(SpriteBatch batch) {
		...
		for(Texture t : _blank){
			batch.draw(t, ChupaChupGame.x, ChupaChupGame.y, WIDTH, HEIGHT);
		}
		for(MyActor x : _xs){
			x.draw(batch);
		}
	...
	private void drawPoint(SpriteBatch batch) {
		batch.draw(_texPoint, ChupaChupGame.x, ChupaChupGame.y, WIDTH, HEIGHT);
		if(_point < 10){
			_fontMark.draw(batch, String.valueOf(_point), 485 + ChupaChupGame.x, 530 + ChupaChupGame.y);
		} else if(_point < 100){
			_fontMark.draw(batch, String.valueOf(_point), 465 + ChupaChupGame.x, 530 + ChupaChupGame.y);
		} else {
			_fontMark.draw(batch, String.valueOf(_point), 440 + ChupaChupGame.x, 530 + ChupaChupGame.y);
		}
	}

	@Override
	public boolean keyDown(int keycode) {
		if(ChupaChupGame.GAME_STATUS == GameStatus.ANSWER){
			...
			if(Keys.NUM_1 <= keycode && keycode < (Keys.NUM_1 + currentAnswerSize)){
				...
				ChupaChupGame.GAME_STATUS = GameStatus.START;
			} else if(keycode == Keys.SPACE){
				if(_x < 3){
					_x++;
					Texture xTex = new Texture("x" + _x + ".png");
					_xs.add(new MyActor(xTex, new Rectangle(ChupaChupGame.x, ChupaChupGame.y, WIDTH, HEIGHT), false));
				}
				_sFail.play();
			}
		}
